    ...
    public class MessageProccesing
    {
        public static Object DeserializeObject(string message)
        {
            ...
            if (list.Count > 0)
            {
                ...
                else if (option == Options.CHECK_USER_NAME || option == Options.ADD_FRIEND) return JsonConvert.DeserializeObject<Username>(data);
                else if (option == Options.GET_FRIENDS || option == Options.ACCEPTED_FRIENDS || option == Options.ACTIVE_FRIENDS) return JsonConvert.DeserializeObject<List<Friend>>(data);
